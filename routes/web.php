...
if ($installed === false) {
    ...
    // Route::get('tap-callback', 'device/PaymentController@callback')->name('tap.callback');

    // Route::get('/', 'HomeController@home')->name('home');


// PaymentController
    Route::get('callback', 'PaymentController@callback')->name('tap.callback');

    Route::get('callback', 'PaymentController@callback')->name('tap.callback');

    

    Route::get('test', 'HomeController@testConnection');
...
//  return redirect()->route('home')->with('success','Payment Successfully Made.');
    ...
    Route::get('path/videos/q1/{filename}', 'S3ProxyController@showq1')->where('filename', '.*');

    Route::get('path/videos/q2/{filename}', 'S3ProxyController@showq2')->where('filename', '.*');
